...
#ifndef fpht_STATIC_HASH_FUNCTION
...
#ifndef fpht_STATIC_COMPARE_EQUAL_FUNCTION
	auto config = __fp_hash_table_config(table);
...
#ifndef fpht_STATIC_COPY_FUNCTION
	auto config = __fp_hash_table_config(table);
...
#ifndef fpht_STATIC_SWAP_FUNCTION
	auto config = __fp_hash_table_config(table);
...
#ifndef fpht_STATIC_FINALIZE_FUNCTION
	auto config = __fp_hash_table_config(table);
...
inline static size_t __fpht_double_size_and_rehash(void** table, size_t type_size, size_t failures) {
	...
	for(size_t i = 0; i < size; ++i)
		if(i % 2 == 1) {
			__fpht_swap(*table, tableP + i * type_size, tableP + (new_size - i) * type_size, type_size);
...
#define fp_create_default_hash_table_from_view(type, view) fp_create_hash_table_from_view(type, view, fpht_default_create_config())
...
#define fp_create_default_hash_table_from_array(a) fp_create_hash_table_from_array(a, fpht_default_create_config())
