...
namespace fp {
	...
	public:
		struct config {
			...
			fp_hash_table_equal_function_t compare_function = hash_table::equal_function;
			fp_hash_table_copy_function_t copy_function = hash_table::copy_function;
			fp_hash_table_swap_function_t swap_function = hash_table::swap_function;
			fp_hash_table_finalize_function_t finalize_function = hash_table::finalize_function;
		...
		bool is_hash_table() const {
			return is_fp_hash_table(ptr());
		}

		size_t rehash() {
			return fp_hash_table_rehash(ptr());
		}

		size_t double_size_and_rehash() {
			return fp_hash_table_double_size_and_rehash(ptr());
		}

		T& insert(const T& key, bool assume_unique = false) {
			if(assume_unique) return *fp_hash_table_insert_assume_unique(ptr(), key);
			else return *fp_hash_table_insert(ptr(), key);
		}

		size_t find_position(const T& key) const {
			return fp_hash_table_find_position(ptr(), key);
		}

		bool contains(const T& key) const {
			return fp_hash_table_contains(ptr(), key);
		}

		T* find(const T& key) {
			return fp_hash_table_find(ptr(), key);
		}
		...
		hash_table& remove_at_position(size_t position) {
			fp_hash_table_remove_at_position(ptr(), position);
			return *this;
		}

		hash_table& remove(const T& key) {
			fp_hash_table_remove(ptr(), key);
			return *this;
		}

		size_t find_first_occupied_position(){
			return fp_hash_table_find_first_occupied_position(ptr());
		}

		T* find_first_occupied(){
			return fp_hash_table_find_first_occupied(ptr());
		}

		size_t find_last_occupied_position(){
			return fp_hash_table_find_last_occupied_position(ptr());
		}

		T* find_last_occupied (){
			return fp_hash_table_find_last_occupied(ptr());
		}

		size_t occupied_size() {
			return fp_hash_table_occupied_size(ptr());
		}

		hash_table& finalize() {
			fp_hash_table_finalize(ptr());
		}

		hash_table& clear() {
			fp_hash_table_clear(ptr());
		}

		inline void free(bool nullify = true) {
			fp_hash_table_free(ptr());
	...
	public:
		...
		Value& insert(const Key& key, bool assume_unique = false) {
			auto& p = super::insert({key, {}}, assume_unique);
			return p.second;
		}
