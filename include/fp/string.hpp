...
namespace fp {
	...
	struct string_view: public view<char> {
		using __base = view<char>;
		using __base::__base;
		string_view(char* str) : string_view(from_cstr(str)) {}
		string_view(const char* str) : string_view(from_cstr(str)) {}
		string_view(fp_string_view view) : __base(fp_view_data(char, view), fp_view_size(view)) { }
		...
		inline string_view subview(size_t start, size_t length) { return {__base::subview(start, length)}; }
		inline string_view subview_max_size(size_t start, size_t length) { return {__base::subview_max_size(start, length)}; }
		inline string_view subview(size_t start) { return {__base::subview(start)}; }
		inline string_view subview_start_end(size_t start, size_t end) { return {__base::subview_start_end(start, end)}; }
...
#ifdef FP_OSTREAM_SUPPORT
		string_view(std::string_view v) : __base((char*)v.data(), v.size()) {}
	...
	struct string_crtp_common {
		...
		Dynamic replace_range(const string_view with, size_t start, size_t len) const {
			return Dynamic{fp_string_view_replace_range(to_view(), with, start, len)};
		}
		...
		Dynamic replace(const string_view find, const string_view replace, size_t start = 0) const {
			return Dynamic{fp_string_view_replace(to_view(), find, replace, start)};
		}
	...
	struct string_crtp : public string_crtp_common<Derived, Derived> {
		...
		Derived concatenate(const string_view o) const { return Derived{fp_string_view_concatenate(this->to_view(), o)}; }
	...
	namespace builder {
		struct string: public raii::string {
			...
			inline string& operator<<(T value) {
				concatenate_inplace(fp::string{raii::format("{}", value).release()});
				return *this;
			}
...
inline static fp::string operator ""_fp(const char* p, size_t size) {
	return fp::string_view{fp_string_view_literal((char*)p, size)}.make_dynamic();
}
